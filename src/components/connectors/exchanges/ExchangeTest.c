...
int __exchangeTest_connect(Exchange* exchange){
  ...
  HttpsRequest_prerequest prefill = {
      .url = exchange->url->full,
      .method = "GET",
      .body = "",
      .headers = headers,
      .header_count = 5,
  ...
  HttpsResponse *response = malloc(sizeof(HttpsResponse));
  HttpsRequest *request = httpsRequest_constructor(prefill);

  status = ws_establish_handshake(&exchange->connection, request, response);
  if (status) {
...
int __exchangeTest_authenticate(Exchange* exchange, void *credentials){
  ...
  HttpsRequest_prerequest pre = {
    .url = url,
    .method = HTTP_METHODS_POST,
    .body = body,
    .headers = headers,
    .header_count = sizeof(headers) / sizeof(headers[0]),
  };

  HttpsRequest *req = httpsRequest_constructor(pre);
  HttpsResponse res;
